...
export default function RootError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
    console.error('[root] erro:', error)
  }, [error])

  return (
    <div className="min-h-screen flex items-center justify-center p-6 page-bg">
      <div className="card rounded-2xl p-8 max-w-md w-full text-center page-enter">
        <div
        ...
        <p className="text-muted-foreground text-sm mb-1">Ocorreu um erro inesperado. Tente novamente.</p>
        {error?.digest && (
