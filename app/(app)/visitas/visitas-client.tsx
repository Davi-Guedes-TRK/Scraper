...
export function VisitasClient() {
  ...
  // ─── Painel da rota (compartilhado mobile/desktop) ────────────────────────
  const routePanel = (
      ...
      <div className="px-4 py-3 border-b border-[#d0d7de] bg-white">
        ...
        <div className="flex gap-2 mt-3">
          <button onClick={getLocation} disabled={locLoading}
            className="flex-1 text-xs font-medium bg-primary hover:bg-primary-h disabled:opacity-50 text-white px-3 py-2.5 rounded-lg transition-colors flex items-center justify-center gap-1.5 cursor-pointer">
            {locLoading
          ...
          <button onClick={openAllMaps} disabled={!route.length}
            className="flex-1 text-xs font-medium bg-green-600 hover:bg-green-500 disabled:opacity-40 text-white px-3 py-2.5 rounded-lg transition-colors cursor-pointer">
            Abrir no Maps
          </button>
        </div>
