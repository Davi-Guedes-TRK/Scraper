"""
Scraper: elar.com.br → Supabase (tabela empreendimentos)
Uso: python scripts/elar_sync.py
"""
...
def collect_slugs() -> list[tuple[str, str]]:
    soup = get(LIST)
    slugs, seen = [], set()
    for a in soup.select("a[href*='/empreendimento'], a[href*='/imovel'], a[href*='/novos']"):
        href = a.get("href", "")
        m = re.search(r"/(?:empreendimento|empreendimentos|imovel|novos)/([^/?#]+)/?", href)
        if not m:
            continue
