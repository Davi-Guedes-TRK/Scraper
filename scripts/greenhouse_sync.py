"""
Scraper: greenhouseimob.com.br → Supabase (tabela empreendimentos)
Uso: python scripts/greenhouse_sync.py
"""
...
FONTE  = "greenhouse"
BASE   = "https://greenhouseimoveis.com.br"
LIST_CANDIDATES = [
    f"{BASE}/imoveis/",
    f"{BASE}/empreendimentos-destaques",
    f"{BASE}/lancamentos/",
]
HEADERS = {
...
_SKIP = {"imoveis", "empreendimentos-destaques", "lancamentos", "cliente",
         "solicite-seu-imovel", "cadastre-seu-imovel", "contato", "blog",
         "admin", "Array", ""}
    ...
    for list_url in LIST_CANDIDATES:
        ...
        for a in soup.select("a[href]"):
            ...
            if not full.startswith(BASE + "/"):
                continue
            path = full[len(BASE):].strip("/")
            if "/" in path or "?" in path or not path or path in _SKIP:
                continue
            if path in seen:
                continue
            seen.add(path)
            slugs.append((path, full))
        if slugs:
            break
