"""
Scraper: paulooctavio.com.br → Supabase (tabela empreendimentos)
Uso: python scripts/paulo_octavio_sync.py
"""
...
FONTE  = "paulo_octavio"
BASE   = "https://www.paulooctavio.com.br"
LIST   = f"{BASE}/novos"
BAIRROS_ALVO = {
    "lago sul", "lago norte", "asa sul", "asa norte",
    "noroeste", "sudoeste", "park way", "parkway", "park sul",
}
HEADERS = {
...
def collect_slugs() -> list[tuple[str, str]]:
    soup = get(LIST)
    slugs, seen = [], set()
    for a in soup.select("a[href*='/empreendimento-']"):
        href = a.get("href", "")
        m = re.search(r"/(empreendimento-\d+-detalhes-[^/?#]+)", href)
        if not m:
            continue
...
def scrape(slug: str, url: str) -> dict:
    soup = get(url)
    ...
    bairros_conhecidos = [
        ...
        "Jardim Botânico", "Águas Claras", "Taguatinga",
    ]
    ...
    if precos:
        vals = sorted(
            float(p.replace(".", "").replace(",", ".")) for p in precos
            if float(p.replace(".", "").replace(",", ".")) > 50_000
        )
        if vals:
