"""
Scraper: zapimoveis.com.br → Supabase (tabela imoveis_zap)
Usa Glue API JSON (sem browser). Fallback: Playwright.
Uso: python scripts/zap_sync.py
"""
...
def fetch_page_api(bairro: str, offset: int) -> dict | None:
    params = {
        ...
        "sort":                "updatedAt desc",
        "fields":              "search",
        "includeFields":       "search(result(listings(listing(displayAddressType,amenities,usableAreas,constructionStatus,listingType,description,title,stamps,createdAt,floors,unitTypes,nonActivationReason,providerId,openDate,propertyType,unitSubTypes,unitsOnTheFloor,legacyId,id,portal,unitFloor,parkingSpaces,updatedAt,address,suites,publicationType,externalId,bathrooms,usableArea,bedrooms,pricingInfos,showPrice,resale,buildings,capacityLimit,status),account(id,name,licenseNumber,showAddress,legacyZapId,phones,whatsappNumber)))",
    }
    try:
...
def fetch_with_playwright(bairro: str) -> list[dict]:
    ...
    url  = f"{BASE_URL}/aluguel/imoveis/df+brasilia+{slug}/"
    rows = []
    ...
    with sync_playwright() as pw:
        ...
        for pg in range(1, 11):
            ...
            cards = page.query_selector_all("[data-type='property'],[class*='property-card'],[class*='listing-item']")
            if not cards:
                break
