"""
Scraper: rivaincorporadora.com.br → Supabase (tabela empreendimentos)
Uso: python scripts/riva_sync.py
"""
...
FONTE  = "riva"
BASE   = "https://www.rivaincorporadora.com.br"
LIST   = f"{BASE}/imoveis/"
HEADERS = {
...
# Siglas dos 26 estados + slugs de navegação que o site Riva usa como filtros
_SKIP = {
...
def collect_slugs() -> list[tuple[str, str]]:
    soup = get(LIST)
    slugs, seen = [], set()
    for a in soup.select("a[href*='/imovel'], a[href*='/empreendimento'], a[href*='/imoveis/']"):
        href = a.get("href", "")
        m = re.search(r"/(?:imovel|imoveis|empreendimento|empreendimentos)/([^/?#]+)/?", href)
        if not m:
            continue
        slug = m.group(1).strip("/")
        if not slug or slug in seen or slug in _SKIP:
            continue
        seen.add(slug)
...
def is_df(page_text: str) -> bool:
    ...
    return any(kw in tl for kw in [
        ...
        "águas claras", "aguas claras", "taguatinga",
    ])
...
def scrape(slug: str, url: str) -> dict | None:
    ...
    if not is_df(page_text):
        log.info("Ignorado (fora do DF): %s", slug)
        return None
